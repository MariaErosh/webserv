...
namespace Core {
		...
		std::string   PageGenerator::generateErrorPage(Http::StatusCode status_code) {
			...
			return Utils::File::readFile((PageGenerator::error_pages_dir_
		...
		std::string  PageGenerator::generateIndexPage(const std::string& absolute_path,
			...
			Utils::Logger::debug("PageGenerator::generateIndexPage");
			Utils::Logger::debug("Index absolute_path : " + absolute_path);
			Utils::Logger::debug("Index request uri   : " + request_uri);
